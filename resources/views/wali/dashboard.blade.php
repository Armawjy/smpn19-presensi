...
@section('title', 'Wali Dashboard')
@section('page_title', 'Monitoring Wali Siswa')
@section('page_subtitle', 'Pantau perkembangan dan kehadiran anak Anda')
...
@if(!$siswa)
    <div class="bg-white border rounded-2xl p-6 text-center text-gray-500 font-semibold shadow-sm">
        ...
        <p>Hubungan dengan data siswa belum terhubung. Harap hubungi administrator.</p>
    </div>
@else
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <x-card>
            <div class="flex items-center space-x-4">
                <div class="w-16 h-16 rounded-2xl bg-indigo-50 text-indigo-500 flex items-center justify-center text-2xl border">
                    <i class="fa-solid fa-child"></i>
                </div>
                <div>
                    <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider">Nama Anak Anda</h4>
                    <h3 class="text-lg font-bold text-gray-900 mt-0.5">{{ $siswa->user->name }}</h3>
                    <p class="text-xs text-gray-500 mt-0.5">NIS. {{ $siswa->nis }} • Kelas {{ $siswa->kelas->nama_kelas }}</p>
                </div>
            </div>
        </x-card>

        <!-- Persentase Kehadiran Anak -->
        <x-card>
            @php
                $totalPresensi = \App\Models\Presensi::where('siswa_id', $siswa->id)->count();
                $hadirCount = \App\Models\Presensi::where('siswa_id', $siswa->id)->where('status', 'hadir')->count();
                $persentaseKehadiran = $totalPresensi > 0 ? round(($hadirCount / $totalPresensi) * 100) : 0;
            @endphp
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-bold text-gray-400 uppercase tracking-wider">Kehadiran Kelas Anak</p>
                    <h3 class="text-3xl font-black text-slate-850 mt-1">{{ $persentaseKehadiran }}%</h3>
                </div>
                <div class="w-12 h-12 rounded-2xl bg-emerald-50 text-emerald-500 flex items-center justify-center text-xl border">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-150 p-6 lg:col-span-1 shadow-sm">
            <h3 class="text-base font-bold text-gray-800 mb-4 border-b pb-2 flex items-center">
                <i class="fa-solid fa-calendar-day text-primary-500 mr-2"></i> Jadwal Anak Hari Ini ({{ $hariIni }})
            </h3>
            <ul class="space-y-3.5">
                ...
                @empty
                    <li class="text-center text-gray-400 font-medium text-sm py-6">Tidak ada jadwal pelajaran hari ini.</li>
                @endforelse
            </ul>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-150 p-6 lg:col-span-2 shadow-sm">
            <h3 class="text-base font-bold text-gray-800 mb-4 border-b pb-2 flex items-center">
                <i class="fa-solid fa-clock-rotate-left text-primary-500 mr-2"></i> Kehadiran Terakhir Anak
            </h3>
            <x-table>
                <x-slot name="thead">
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-4 py-6 text-center text-gray-400">Belum ada catatan presensi anak.</td>
                    </tr>
                @endforelse
            </x-table>
        </div>
