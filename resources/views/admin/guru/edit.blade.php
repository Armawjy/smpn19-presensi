...
@section('page_subtitle', 'Ubah data dan kelayakan profil guru')
        ...
        <form action="{{ route('admin.guru.update', $guru->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="nip" value="{{ old('nip', $guru->nip) }}" required class="w-full border-gray-200 focus:border-primary-500 focus:ring-primary-500 rounded-xl text-sm">
                </div>

                <!-- Nama Lengkap -->
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Nama Lengkap (Beserta Gelar)</label>
                    <input type="text" name="name" value="{{ old('name', $guru->user->name) }}" required class="w-full border-gray-200 focus:border-primary-500 focus:ring-primary-500 rounded-xl text-sm">
                </div>

                <!-- Email -->
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Email</label>
                    <input type="email" name="email" value="{{ old('email', $guru->user->email) }}" required class="w-full border-gray-200 focus:border-primary-500 focus:ring-primary-500 rounded-xl text-sm">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="no_hp" value="{{ old('no_hp', $guru->no_hp) }}" class="w-full border-gray-200 focus:border-primary-500 focus:ring-primary-500 rounded-xl text-sm">
                </div>
            ...
            <div>
                ...
                <textarea name="alamat" rows="3" class="w-full border-gray-200 focus:border-primary-500 focus:ring-primary-500 rounded-xl text-sm">{{ old('alamat', $guru->alamat) }}</textarea>
            </div>
            ...
            <div class="flex items-center space-x-4">
                @if($guru->user->foto)
                    <div class="shrink-0">
                        <img src="{{ asset('storage/' . $guru->user->foto) }}" class="w-16 h-16 rounded-2xl object-cover border" alt="Foto">
                    </div>
                @endif
                <div class="flex-1">
